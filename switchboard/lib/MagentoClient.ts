...
export class MagentoClient {
  ...
  constructor(switchContext: SwitchContext) {
    try {
      ...
      const si = this.SwitchContext.merchant.serviceIntegrations.find((integration) => {
        return integration.type === 'MAGENTO';
      });
      if (!si) throw new Error('No Magento Service Integration defined on this merchant');

      // TODO: refactor this to handle cases where these assumptions cannot be made
      this.apiUrl = `${(
        this.SwitchContext.merchant.storefrontUrl || `https://${this.SwitchContext.merchant.domain}`
  ...
  public getOrder = async (
    ...
    try {
      ...
      if ((await handleApiError(e, async () => this.getOrder(orderId, retryConfig), retryConfig)) === false) {
  ...
  public getOrderByIncrementId = async (
    ...
    try {
      ...
      if (
        (await handleApiError(e, async () => this.getOrderByIncrementId(incrementId, retryConfig), retryConfig)) ===
        false
      ) {
  ...
  public getCustomer = async (
    ...
    try {
      ...
      if ((await handleApiError(e, async () => this.getCustomer(customerId, retryConfig), retryConfig)) === false) {
  ...
  public getTransaction = async (
    ...
    try {
      ...
      if (
        (await handleApiError(e, async () => this.getTransaction(transactionId, retryConfig), retryConfig)) === false
      ) {
